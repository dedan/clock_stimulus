...
%    $Revision: 1.10.2.5 $  $Date: 2004/12/01 19:47:57 $

% Get the property names (names) and property values (val).
unsortednames = fieldnames(out);
unsortedval = struct2cell(out);

[names index] = sort(lower(unsortednames));
for i = 1:length(index)
    val{i} = unsortedval{index(i)};
    names{i} = unsortednames{index(i)};
end
...
if ~isempty(deviceprops)
   ...
   adaptor = daqhwinfo(parent,'AdaptorName');
   
   % Create device specific heading.
   fprintf(['        ' upper(adaptor) ' specific properties:\n']);
