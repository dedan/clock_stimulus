...
%    $Revision: 1.13.2.5 $  $Date: 2004/12/01 19:47:13 $
...
function [store_prop,bad_prop,errflag]=localStructSet(obj,structure,all_prop,store_prop)
...
% Save the real error in case one of internal functions happens to
% cause an error that changes it.
savedlasterr = lasterr;

errmsg = lower(savedlasterr);
...
try
 ...
 if any(findstr('changing the', errmsg)) || any(findstr('type mismatch', errmsg)) || ... 
      any(findstr('enum', errmsg)) || any(findstr('enumerated', errmsg)) 
   ...
   if strcmp(propstruct.ReadOnly,'always')
      ...
      return;
   elseif strcmpi(propstruct.ReadOnly,'whileRunning') && strcmpi(get(x.uddobject, 'Running'), 'on')
...
if findstr('parameter must be a scalar handle', errmsg)
   lasterr(sprintf(['Object must be a single object for property: ''' prop '''.']));
   return;
end

% Restore the real last error.
