...
// $Revision: 1.1.6.3 $  $Date: 2004/12/01 19:51:07 $
...
CComPtr<IProp> SoundAD::RegisterProperty(LPWSTR name, DWORD dwUser)
{
    ...
    if (prop==NULL) {
        _RPT1(_CRT_ERROR,"Winsound: Invalid property: '%s'.", name);
        return NULL;
    }
...
{
    // prefix the Windows Multimedia error code with "Windows Multimedia: "
    TCHAR str[MAXERRORLENGTH+1] = "Windows Multimedia: ";
    size_t cbstr = strlen(str);
    waveInGetErrorText(err,str + cbstr,MAXERRORLENGTH - cbstr);
    return Error(str);
}
...
STDMETHODIMP SoundAD::Start ()
{
    ...
    if (!_waveInDevice.Ok())
    {
        // Seems that some devices return the wrong error code when the device is
        // busy.  If we didn't get a WAVERR_BADFORMAT error earlier, interpret this
        // error code as IN_USE
        if (_waveInDevice.isInUse() || _waveInDevice.GetError() == WAVERR_BADFORMAT)
            return E_DEVICE_IN_USE;            
        else
            return WaveInError(_waveInDevice.GetError());
       // return FALSE;
    }
    _isStarted = true;
