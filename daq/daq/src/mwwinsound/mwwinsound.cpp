...
// $Revision: 1.1.6.3 $  $Date: 2004/12/01 19:51:10 $
                             ...
                             void __RPC_FAR *__RPC_FAR *ppIDevice)
{
    ...
    else if ( InlineIsEqualGUID(__uuidof(ImwOutput),riid))
    {
        ...
        return Error(E_SUBSYSTEM_NOT_SUPPORTED);
...
{
    TCHAR lpMsgBuf[401] = "Winsound: ";
    size_t cbMsgBuf = strlen(lpMsgBuf);
    
    // Append the resource string on to the Winsound prefix.
    int size=LoadString(_Module.GetResourceInstance(), code, lpMsgBuf + cbMsgBuf,400 - cbMsgBuf);
