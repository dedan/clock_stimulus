...
// $Revision: 1.6.4.10 $  $Date: 2004/12/18 07:35:09 $
        ...
        switch (_triggerType) 
    {
        ...
        case HW_DIGITAL:
            {	
                ...
                // Geck 80530:  Add support for rising and falling trigger
                // conditions.
                u32 sourceSpec;
                switch(_triggerCondition)
                {
...
//    }
    ...
    if (Stopped && !_UseNSCANSCallback)
    {
        ...
        // Reset the circular buffer after we read it out, or we won't be able to read from it again.
        GetCircBuffer()->Reset();
    }
    else
    {
...
HRESULT Cnia2d::Open(IDaqEngine *engine,int id,DevCaps *DeviceCaps)
{
   ...
   // _engine = engine;
	...
	_sampleRate = GetDevCaps()->FindRate(GetTargetSampleRate(), &_scanTB, &_scanInterval);
...
{    
        ...
        case CHAN_SKEW_MANUAL:
			// need to determine if ChannelSkew is a valid setting
			{
                ...
                tAdjustedSampleRate = CalculateSampleRate(tempInterval,tempTB);
...
STDMETHODIMP Cnia2d::SetProperty(long User, VARIANT *newVal)
{
    ...
    if (_Nia2dWnd->IsOpen(_id) && _Nia2dWnd->GetDevFromId(_id) !=this)
        ...
        if (User==(long)&_sampleRate)
        {
			...
			calculatedSampleRate = GetDevCaps()->FindRate(requestedSampleRate, &tempTB, &tempInterval);
...
*/
        ...
        else if (User==(long)&_triggerType)
	{
        ...
        // conditions.
            ...
            _triggerCondition->AddMappedEnumValue(TRIG_RISING_EDGE, L"PositiveEdge");    
            _triggerCondition->AddMappedEnumValue(TRIG_FALLING_EDGE, L"NegativeEdge");    
            _triggerCondition->put_Value(CComVariant(TRIG_FALLING_EDGE));
            _triggerCondition->put_DefaultValue(CComVariant(TRIG_FALLING_EDGE));
