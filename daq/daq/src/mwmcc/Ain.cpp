...
// $Revision: 1.1.6.9 $  $Date: 2004/12/01 19:50:51 $
...
CAin::~CAin()
{
    if (_BoardNum>=0)
    {
        ...
        ATLTRACE2(MCC_UL_CALL,3,"cbStopBackground(_BoardNum = %i, AIFUNCTION);\n",_BoardNum);
    }
...
HRESULT CAin::Open(IUnknown *Interface,long ID)
{
    ...
    ATLTRACE2(MCC_UL_CALL,3,"cbGetConfig (BOARDINFO, _BoardNum = %d, 0, BINUMADCHANS, &chans); result: chans=%d\n",_BoardNum,chans);
    if (chans==0)
        return ERR_XLATE(ERR_SUBSYSTEM_NOT_SUPPORTED);
    ...
    else
    {
        ...
        pChannelSkewMode->AddMappedEnumValue(0, L"None");    
    ...
    pChannelSkew->put_IsReadOnly(true);
...
//    _swRangeConfig = GetFromIni("swRangeConfig", false);
    ...
    // Geck 214725:  Some devices, specifically the PMD-1608FS require that the buffer
    // size be evenly divisible by the packetsize.  If this value is -1, then it's ignored.
    // if it's anything else, the buffers will be rounded up to the next largest multiple of
    // that value.
    _packetSize = GetFromIni("PacketSize", -1);
...
HRESULT CAin::SetDaqHwInfo()
{
    ...
    ATLTRACE2(MCC_UL_CALL,3,"cbGetConfig (BOARDINFO, _BoardNum = %d, 0, BINUMADCHANS, &_MaxChannels); result: _MaxChannels=%d\n",_BoardNum,_MaxChannels);
    ...
    ATLTRACE2(MCC_UL_CALL,3,"cbGetConfig (BOARDINFO, _BoardNum = %d, 0, BIADCFG, &temp); result: temp=%d\n",_BoardNum,temp);
    ...
    ATLTRACE2(MCC_UL_CALL,3,"cbGetBoardName(_BoardNum = %i, BoardName); result BoardName = %s\n",_BoardNum,BoardName);
...
//    var = support16bit ? VT_UI2 : VT_UI1;
    ...
    int range;
        ...
        ATLTRACE2(MCC_UL_CALL,3,"cbGetConfig (BOARDINFO, _BoardNum = %d, 0, BIRANGE, &range); result: range=%d\n",_BoardNum,range);
        if (!status && range>0)
        {
...
HRESULT     CAin::GetSingleValue(int chan,unsigned short *loc)
{
    ...
    CBI_CHECK(cbAIn(_BoardNum,
        ...
        _chanRange[chan],
        loc));
    ATLTRACE2(MCC_UL_CALL,3,"cbAIn(_BoardNum = %i, _chanList[chan] = %i, _chanRange[chan] = %i, loc); result: loc=%d\n",_BoardNum,_chanList[chan], _chanRange[chan], *loc );
    return S_OK;
}
#endif
HRESULT CAin::GetSingleValues(VARIANT * Values)
{
    ...
    {    
        ...
        CBI_CHECK(cbAIn(_BoardNum,
             ...
             _chanRange[i],
             &binarray[i]));
        ATLTRACE2(MCC_UL_CALL,3,"cbAIn(_BoardNum = %i, _chanList[i] = %i, _chanRange[i] = %i, binarray[i]);\n",_BoardNum,_chanList[i], _chanRange[i]);
//        if (_convertData)
//            binarray[i]>>=4;
    }
...
HRESULT CAin::UpdateRateAndSkew(long newskewmode,double newrate)
{
    ...
    // If simultaneous sample and hold (non-scanning) device.
    if ( !_scanning )
    {
        // Channel skew is zero, rate is rounded.
        newskew = 0;
        newrate=RoundRate(newrate);
    }
    // If ClockSource is 'Software', only one channel, or ChannelSkewMode is 'Minimum'
    else if ( _UseSoftwareClock || _nChannels<=1 || newskewmode==BURSTMODE )
    {
        // Channel skew is based on settle time, rate is rounded.
        newskew=_settleTime*1e-6;
        newrate=RoundRate(newrate);
    }
    else
    {
        // Channel skew is based on aggregate sample rate.
	...
	// If a scanning device and ClockSource is not 'Software', 
	// verify that we have not exceeded the maximum aggregate sample rate.
    if ( _scanning && !_UseSoftwareClock )
	{
...
HRESULT CAin::Start()
{
    ...
    long skewmode = pChannelSkewMode;
    /* Geck 228416: turn OFF BURSTMODE when acquiring on a single channel
        This has been confirmed as bug in Instacal by Measurement Computing.
        The problem is related to our use of the BURSTMODE flag when doing a
        single channel acquisition.  In a single channel acquisition, it's
        meaningless, but it confused Universal Library, which causes the problem
        with the MCC PCI-DAS64/M1/16.  Setting ChannelSkewMode to Equisample also
        avoids the problem, or simply using more than one channel.  However, for
        safety's sake, we turn OFF BURSTMODE when acquiring on a single channel. */
    if(_nChannels<=1)
    {
        skewmode = 0;
    }

    _Options = BACKGROUND | pTransferMode | skewmode | GET_ADAPTOR_ENUM_VALUE(pClockSource);
    ...
    if (pTriggerType &  TRIG_TRIGGER_IS_HARDWARE)
    {
        ...
        ATLTRACE2(MCC_UL_CALL,3,"cbSetTrigger(_BoardNum = %i, pTriggerCondition = %i, lowt = %i, hit = %i);\n",_BoardNum,pTriggerCondition, lowt, hit );
    ...
    {
        // We're in continuous mode, where the board acquires a block of data, and transmits it
        // while acquiring the next block of data.
		_Bursting=false;
        _Options |= CONTINUOUS;
        if (BufferSize<8*1024)
        {
            // This is magic:  It's not clear why 8K is magic.
            // This calculates the largest value that is a multiple of buffersize,
            // but less than 8K.
            BufferSize*=8*1024/BufferSize;
        }

        // Geck 214725:  Some devices, specifically the PMD-1608FS require that the buffer
        // size be evenly divisible by the _packetSize.  If this value is -1, then it's ignored.
        if(_packetSize > 0)
        {
            // _packetSize is valid: the buffer size will be rounded up to the next
            // largest multiple of that _packetSize.
            BufferSize = ((BufferSize/_packetSize) + 1) * _packetSize;
        }
    }
	...
	{
		// If we don't know the number of points (operating in continuous mode), or
		// we know the number of points (burst mode) but the total points exceed the fifo size,
		// the Universal Library will be attempting continuous mode acquisition.
		// However, we have exceeded the maximum continuous rate.
		if ( TotalPoints < 0 || TotalPoints > _FifoSize )
		{
    ...
    if (_chanGainQueue)
    {
        ...
        ATLTRACE2(MCC_UL_CALL,3,"cbALoadQueue(_BoardNum = %i, _chanList[0], _chanRange[0], _nChannels = %i);\n",_BoardNum,_nChannels );
    }
    return S_OK;
}
// Description:
...
HRESULT CAin::Trigger()
{
    ...
    ATLTRACE2(MCC_UL_CALL,3,"cbAInScan(_BoardNum = %i, _chanList[0] = %i, _chanRange[_nChannels-1] = %i, _CircBuff.GetBufferSize() = %i, &_SampleRate = %i, _chanRange[0] = %i, _CircBuff.GetPtr() = 0x%08X, _Options = %i);\n",
    ...
    ATLTRACE2(MCC_UL_CALL,3,"cbGetStatus(_BoardNum = %i, &Status, &CurCount, &_CurIndex, AIFUNCTION); result: Status=%d CurCount=%d CurIndex=%d\n",_BoardNum,Status, CurCount, _CurIndex );
#endif
    return S_OK;
}
...
STDMETHODIMP CAin::SetConfig(long InfoType, long DevNum, long ConfigItem, long ConfigVal)
{
	// TODO: Add your implementation code here
        ...
        ATLTRACE2(MCC_UL_CALL,3,"cbSetConfig (InfoType = %d, _BoardNum = %d, DevNum = %d, ConfigItem = %d, ConfigVal = %d); result: ConfigVal=%d\n",InfoType,_BoardNum,DevNum,ConfigItem,ConfigVal);
	return S_OK;
}
...
STDMETHODIMP CAin::GetConfig(long InfoType, long DevNum, long ConfigItem, long *ConfigVal)
{
	// TODO: Add your implementation code here
        ...
        ATLTRACE2(MCC_UL_CALL,3,"cbGetConfig (InfoType = %d, _BoardNum = %d, DevNum = %d, ConfigItem = %d, ConfigVal); result: ConfigVal=%d\n",InfoType,_BoardNum,DevNum,ConfigItem,*ConfigVal);
	return S_OK;
}
...
STDMETHODIMP CAin::AIn(long Channel, long Range, unsigned short *DataValue)
{
	// TODO: Add your implementation code here
        ...
        ATLTRACE2(MCC_UL_CALL,3,"cbAIn(_BoardNum = %i, Channel = %i,Range = %i, DataValue); result: loc=%d\n",_BoardNum,Channel,Range, *DataValue );
	return S_OK;
}
...
STDMETHODIMP CAin::C8254Config(long CounterNum, long Config)
{
	// TODO: Add your implementation code here
        ...
        ATLTRACE2(MCC_UL_CALL,3,"cbC8254Config(_BoardNum = %i, CounterNum = %i,Config=%d\n",_BoardNum,CounterNum, Config );
	return S_OK;
}
...
STDMETHODIMP CAin::CLoad(VARIANT RegName, unsigned long LoadValue)
{
	// TODO: Add your implementation code here
        ...
        ATLTRACE2(MCC_UL_CALL,3,"cbCLoad(_BoardNum = %i, regvalue = %i,LoadValue=%d\n",_BoardNum,regvalue, LoadValue );
	return S_OK;
}
