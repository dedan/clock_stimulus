...
// $Revision: 1.1.6.4 $  $Date: 2004/08/11 22:07:22 $
...
HRESULT CDio::Open(IUnknown *Interface,long ID)
{
    ...
    int devices=0;
    CBI_CHECK(cbGetConfig (BOARDINFO, _BoardNum, 0, BIDINUMDEVS, &devices));
    if (devices==0)
...
HRESULT CDio::WriteValues(LONG NumberOfPorts, LONG * PortList, ULONG * Data, ULONG * Mask)
{
    ...
    for (int i=0;i<NumberOfPorts;i++)
    {
        ...
        else if(Mask[i]==(0xff >> (8 - getPortLines(PortList[i]))))
        {
            // Use port write
        ...
        {		
            ...
            int numLines = getPortLines(PortList[i]);
...
HRESULT CDio::SetPortDirection(LONG Port, ULONG DirectionValues)
{
    ...
    if ((DirectionValues==0) || (DirectionValues==(0xff >> (8 - getPortLines(Port)))))
    {
    ...
    else 
    {
        ...
        int numLines = getPortLines(Port);
        for (int iLoop=0; iLoop<numLines; iLoop++)
        {
